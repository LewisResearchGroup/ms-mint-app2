...
This renders an 80 (compounds) x 100 (samples) mock matrix using the stock
Clustergrammer UI (clustergrammer.js) inside a tiny Dash app. No custom
overrides—just the required assets.
...
# clustergrammer-py targets older pandas and uses .ix; provide a small shim.
class _IxShim:
...
<html>
  <head>
    ...
    <!-- Use the exact assets from the official Clustergrammer demo -->
...
def make_app(matrix: pd.DataFrame) -> Dash:
    ...
    app.layout = html.Div(
        [
            ...
            html.H4("Data preview (first 8 rows x 6 cols)"),
