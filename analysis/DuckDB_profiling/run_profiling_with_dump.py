"""
Profiling runner that can use baseline or pruned ms1 query, emit profiling JSON, and optionally dump
query outputs for result comparison. Does not modify existing profiling scripts.
"""
...
# Baseline ms1 query (no file-level slice)
MS1_QUERY_BASELINE = """
...
# Pruned ms1 query (slices ms1_data by batch files)
MS1_QUERY_PRUNED = """
INSERT INTO chromatograms (peak_label, ms_file_label, scan_time, intensity, ms_type)
WITH batch_pairs AS (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Profile DuckDB batches (baseline or pruned ms1) with optional result dump.")
    ...
    parser.add_argument("--variant", choices=["baseline", "pruned"], default="baseline", help="ms1 query variant to use")
    ...
    parser.add_argument("--output", default=None, help="Profiling output filename (JSON). Defaults to <query>_<variant>_<start>-<end>.json")
    ...
    parser.add_argument("--dump-results", default=None,
                        help="If set, path to write query output (parquet by default) for comparison.")
    parser.add_argument("--dump-format", choices=["parquet", "csv", "json"], default="csv",
                        help="Format for dumped results when --dump-results is set.")
...
def main():
    ...
    if args.output:
        ...
        profile_path = out_dir / f"{args.query}_{args.variant}_{args.start_id}-{args.end_id}.json"

    sql = {"ms1": MS1_QUERY_PRUNED if args.variant == "pruned" else MS1_QUERY_BASELINE,
           "ms2": MS2_QUERY,
           "results": RESULTS_QUERY}[args.query]
    ...
    try:
        ...
        elapsed = run_with_profile(
            ...
            dump_table=dump_table if args.dump_results else None,
        )
    finally:
        con.close()
